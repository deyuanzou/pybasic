...
class Parser:
    ...
    def cdt(self):
        res = ParseResult()
        all_cases = res.register(self.cdt_cases('if'))
        if res.error:
            return res
    ...
    def cdt_b_or_c(self):
        ...
        if self.current_tok.matches(TT_KEYWORD, 'elif'):
            all_cases = res.register(self.cdt_b())
            if res.error:
                return res
            cases, else_case = all_cases
        else:
            else_case = res.register(self.cdt_c())
            if res.error:
                return res

        return res.success((cases, else_case))

    def cdt_cases(self, case_keyword):
        res = ParseResult()
        cases = []
        else_case = None
        ...
        if self.current_tok.type == TT_NEWLINE:
            ...
            if res.error:
                return res
            cases.append((condition, stms, True))

            if self.current_tok.matches(TT_KEYWORD, 'end'):
                res.register_advancement()
                self.advance()
            else:
                all_cases = res.register(self.cdt_b_or_c())
                if res.error:
                    return res
    ...
    def power(self):
        return self.bi_op(self.call, (TT_POW,), self.factor)
    ...
    def stms(self):
        ...
        while True:
            ...
            if not more_stms:
                break
            stm = res.try_register(self.expr())
            if not stm:
